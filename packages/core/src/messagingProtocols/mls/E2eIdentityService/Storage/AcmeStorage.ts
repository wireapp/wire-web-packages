...
import {AuthData, AuthDataSchema, InitialData, InitialDataSchema} from './AcmeStorage.schema';
...
const storeHandle = (handle: string) => AcmeStore.add(HandleKey, handle);
const storeAuthData = (data: AuthData) => AcmeStore.add(AuthDataKey, JSON.stringify(data));
const storeInitialData = (data: InitialData) => AcmeStore.add(InitialDataKey, JSON.stringify(data));
...
const getAndVerifyAuthData = (): AuthData => {
  ...
  return AuthDataSchema.parse(JSON.parse(data));
};
...
const getInitialData = (): InitialData => {
  ...
  return InitialDataSchema.parse(JSON.parse(data));
};
