...
describe('cryptobox.store.CryptoboxCRUDStore', () => {
  ...
  describe('"delete_all"', () => {
    it('deletes everything from the storage', async () => {
      ...
      const aliceIdentity = await new Proteus.keys.IdentityKeyPair();
      const bobIdentity = await new Proteus.keys.IdentityKeyPair();
      const bobLastResortPreKey = await new Proteus.keys.PreKey(Proteus.keys.PreKey.MAX_PREKEY_ID);
  ...
  describe('"load_prekeys"', () => {
    it('loads multiple PreKeys', async () => {
      await Promise.all([
        fileStore.save_prekey(await new Proteus.keys.PreKey(1)),
        fileStore.save_prekey(await new Proteus.keys.PreKey(2)),
        fileStore.save_prekey(await new Proteus.keys.PreKey(3)),
      ]);
  ...
  describe('"save_identity"', () => {
    it('saves the local identity', async () => {
      const ikp = await new Proteus.keys.IdentityKeyPair();
  ...
  describe('"session_from_prekey"', () => {
    it('saves and caches a valid session from a serialized PreKey bundle', async () => {
      ...
      const bob = await new Proteus.keys.IdentityKeyPair();
      const preKey = await new Proteus.keys.PreKey(Proteus.keys.PreKey.MAX_PREKEY_ID);
