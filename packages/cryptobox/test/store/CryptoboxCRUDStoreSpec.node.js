...
describe('cryptobox.store.CryptoboxCRUDStore', () => {
  ...
  afterEach(async () => {
    await fs.remove(TEMP_DIRECTORY);
  });
  ...
  describe('"update_session"', () => {
    it('updates an already persisted session', async () => {
      ...
      const proteusSession = await Proteus.session.Session.init_from_prekey(aliceIdentity, bobPreKeyBundle);
      let fileStoreSession = await fileStore.create_session(sessionId, proteusSession);

      expect(fileStoreSession.local_identity.public_key.fingerprint()).toBe(aliceIdentity.public_key.fingerprint());
      expect(fileStoreSession.remote_identity.public_key.fingerprint()).toBe(bobIdentity.public_key.fingerprint());
      expect(fileStoreSession.version).toBe(1);

      fileStoreSession.version = 2;
      await fileStore.update_session(sessionId, fileStoreSession);
      fileStoreSession = await fileStore.read_session(aliceIdentity, sessionId);

      expect(fileStoreSession.local_identity.public_key.fingerprint()).toBe(aliceIdentity.public_key.fingerprint());
      expect(fileStoreSession.remote_identity.public_key.fingerprint()).toBe(bobIdentity.public_key.fingerprint());
      expect(fileStoreSession.version).toBe(2);
    });
  });
