...
describe('PreKeyBundle', () => {
  it('creates a bundle', done => {
    Promise.all([Proteus.keys.IdentityKeyPair.new(), Proteus.keys.PreKey.new(1)]).then(([id_pair, prekey]) => {
  ...
  it('creates a valid signed bundle', done => {
    Promise.all([Proteus.keys.IdentityKeyPair.new(), Proteus.keys.PreKey.new(1)]).then(([id_pair, prekey]) => {
  ...
  it('serialises and deserialise an unsigned bundle', done => {
    Promise.all([Proteus.keys.IdentityKeyPair.new(), Proteus.keys.PreKey.new(1)]).then(([id_pair, prekey]) => {
      const bundle = Proteus.keys.PreKeyBundle.new(id_pair.public_key, prekey);
  ...
  it('should serialise and deserialise a signed bundle', done => {
    Promise.all([Proteus.keys.IdentityKeyPair.new(), Proteus.keys.PreKey.new(1)]).then(([id_pair, prekey]) => {
      const bundle = Proteus.keys.PreKeyBundle.signed(id_pair, prekey);
      expect(bundle.verify()).toBe(Proteus.keys.PreKeyAuth.VALID);
  ...
  it('should generate a serialised JSON format', done => {
    const pre_key_id = 72;

    Promise.all([Proteus.keys.IdentityKeyPair.new(), Proteus.keys.PreKey.new(pre_key_id)]).then(
      ([identity_key_pair, pre_key]) => {
