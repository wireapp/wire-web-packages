...
export class UserAPI {
   ...
   */
  public getProperty<T extends string>(propertyKey: T): Promise<{[key in T]: string}> {
    const config: AxiosRequestConfig = {
      method: 'get',
      url: `${UserAPI.URL.PROPERTIES}/${propertyKey}`,
    };

    return this.client.sendJSON<{[key in T]: string}>(config).then(response => response.data);
  }
   ...
   */
  public async putProperty<T extends string>(propertyKey: T, propertyData: {[key in T]: string}): Promise<void> {
    const config: AxiosRequestConfig = {
      data: propertyData,
      method: 'put',
      url: `${UserAPI.URL.PROPERTIES}/${propertyKey}`,
