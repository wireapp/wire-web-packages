...
import type {ChangePassword, Delete, QualifiedSelf} from '../self/';
...
export class SelfAPI {
   ...
   */
  public async getSelf(traceStates: TraceState[] = []): Promise<QualifiedSelf> {
    ...
    const response = await this.client.sendJSON<QualifiedSelf>(config);
    return response.data;
  }
