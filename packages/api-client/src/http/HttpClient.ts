...
export class HttpClient extends EventEmitter {
  ...
  public sendProtocolBuffer<T>(
    config: AxiosRequestConfig & {data: Uint8Array},
    isSynchronousRequest: boolean = false,
  ): Promise<AxiosResponse<T>> {
    // The Uint8Array generated by protobuf can sometimes have a buffer that contains shared data.
    // In order to prevent this, we wrap the protobuf data into a new Uint8Array (which will then have a newly allocated buffer)
    config.data = new Uint8Array(config.data);
    config.headers = {
      ...config.headers,
